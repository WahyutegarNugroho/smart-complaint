    ...
    <div className="min-h-screen bg-brand-canvas-soft text-brand-ink font-sans selection:bg-brand-primary/20 transition-colors duration-300 pb-32">
      ...
      <main className="max-w-6xl mx-auto p-4 md:p-8 lg:p-10 space-y-8 md:space-y-12">
        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-3 mb-2">
               ...
               <span className="text-[10px] font-bold text-brand-primary uppercase tracking-[0.2em]">Konfigurasi Akun</span>
            </div>
          ...
          <div className="h-16 w-16 bg-brand-ink dark:bg-brand-primary rounded-2xl flex items-center justify-center text-brand-canvas dark:text-[#0e0f0c] shrink-0 font-bold text-2xl transition-all rotate-3">
             {profile.name?.[0] || '?'}
          </div>
        </section>
        ...
        <section className="pt-8 border-t border-brand-hairline flex flex-col items-center">
          <p className="text-[10px] font-bold text-brand-ink/40 uppercase tracking-[0.2em] mb-6">Sesi Akun</p>
          <form action={logout} className="w-full max-w-sm">
            <button 
              type="submit" 
              className="w-full flex items-center justify-center gap-3 px-8 py-5 rounded-[2rem] bg-red-500/10 text-red-600 dark:text-red-500 font-bold text-xs uppercase tracking-[0.2em] border border-red-500/20 hover:bg-red-500/20 transition-all active:scale-95 cursor-pointer"
            >
