        ...
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-3 mb-2">
               ...
               <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase tracking-normal">Pusat Informasi</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
             ...
             <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 sticky top-10 transition-colors">
                <div className="flex items-center gap-4 mb-8">
                   ...
                   <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-normal transition-colors">Rilis Pengumuman Warga</p>
                   </div>
                </div>

                <form action={createAnnouncement} className="space-y-6">
                   <div>
                      <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-normal mb-2.5 ml-1 transition-colors">Judul Informasi</label>
                      <input 
                        name="title" 
                        required 
                   ...
                   <div>
                      <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-normal mb-2.5 ml-1 transition-colors">Isi Pengumuman</label>
                      <textarea 
                        name="content" 
                        rows={6} 
                        required 
          ...
          <div className="lg:col-span-7 space-y-6 md:space-y-8">
              <div className="flex items-center justify-between px-2">
                 ...
                 <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-normal transition-colors">{totalAnnouncements} Total</span>
              </div>
              ...
              {announcements.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-20 text-center border border-slate-200 dark:border-slate-800 border-dashed flex flex-col items-center justify-center transition-colors">
                   ...
                   <p className="text-slate-400 dark:text-slate-600 text-sm font-bold uppercase tracking-normal">Belum ada pengumuman aktif</p>
                </div>
              ) : (
